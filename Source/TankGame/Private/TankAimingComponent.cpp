...
UTankAimingComponent::UTankAimingComponent()
{
	...
	static ConstructorHelpers::FClassFinder<AActor> BP_MyActor(TEXT("Blueprint'/Game/Blueprints/Projectile/BP_Projectile.BP_Projectile_c'"));
	ProjectileType = BP_MyActor.Class;
...
void UTankAimingComponent::Fire(float addPower)
{
	UE_LOG(LogTemp, Warning, TEXT("Fire!"));
	bool isReadyToFire = (FPlatformTime::Seconds() - LastFireTime) > TankReloadTime;
	if (Barrel == nullptr || ProjectileType == nullptr || !isReadyToFire)
	{
		UE_LOG(LogTemp, Warning, TEXT("Fire is wrong!"));
		return;
	}
